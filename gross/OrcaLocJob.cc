...
OrcaLocJob::OrcaLocJob(const int anId, const vector<int> aDataSelect, Task* aTask) : Job(anId, aDataSelect, aTask), pXMLFragFile_(0) {};
...
int OrcaLocJob::save() {
  ...
  for (set<string>::const_iterator i=(outFiles()).begin(); i!= (outFiles()).end(); i++){
    myValues.str("");
    myValues << task_->Id() << "," << Id() << ",'"<< (*i)<<"'";
    if(LocalDb::instance()->tableSave("AnalyLoc_OutFiles", myKeys.str(), myValues.str())) return EXIT_FAILURE;
  }
