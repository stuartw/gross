...
int Command::acceptOptions(Options _opts) {
  opt_ = _opts;
  return EXIT_SUCCESS;
}
...
int Command::initDb() {  
  ...
  //(3) DbSpec file in home directory
  //(4) DbSpec file defined in $BOSSDIR/BossConfig.clad
  ...
  if(opt_["-dbSpec"].empty()) {
    ...
    if(pc) {
      ...
      char* pc = getenv("HOME");
      File clad(pc+string("/BossConfig.clad")); 
      if(clad.exists()) {
	opt_["-dbSpec"] = clad.fullHandle();
        if(Log::level()>0) cout <<"Taking DbSpec file set in home area "<<opt_["-dbSpec"]<<endl;
      }
      else {	    
