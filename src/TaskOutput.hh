...
public:
  ...
  int setAuto(string dir);
  int cont();
  int cancelAuto();
  int setFinJobs();
  int recoverJobs(const string journalDir);
private:
  int taskId_;
  int jobsNotRetrieved_;
  Task* task_;
  int minJob_;
  int maxJob_;
  vector<Job*> allJobs_;
  vector<Job*> finJobs_;
  vector<Job*> activeJobs_;
  string retrieveScript;
  string retrieveScriptDir;
  string getDbSbox(Job* pJob) const; ///<Read from Db
  string getGridSbox(Job* pJob, string oDir="./") const; ///<Retrieve from Grid
  string getLocalSbox(Job* pJob, const string oDir) const;
  int saveSboxDir(Job* pJob, string sboxDir) const; ///<Save to Db
  int checkOutputLFNs(Job* pJob, string sboxDir);
  int checkJobsLeft();
  int createRetrieveScript(string dir);
