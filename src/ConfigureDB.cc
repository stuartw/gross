...
void ConfigureDB::printUsage() const
{
  ...
  cerr
       << "used to add GROSS tables to BOSS database" << endl
       << endl;
}
int ConfigureDB::execute() {
  ...
  values.push_back("orcaVers VARCHAR(255)");
  ...
  */
  ...
  values.push_back("name VARCHAR(255) NOT NULL PRIMARY KEY");
  values.push_back("script MEDIUMBLOB NOT NULL DEFAULT \"\"");
  if(LocalDb::instance()->tableCreate("Analy_Retrieve", values))
    return EXIT_FAILURE;
  ...
  //values.push_back("TaskID INT");
  //values.push_back("Name VARCHAR(255)");
  //if(LocalDb::instance()->tableCreate("Analy_ExeTar", values))
  //  return EXIT_FAILURE;
  ...
  cout << "GROSS database correctly configured" <<endl;
