...
int Task::delSubFiles(){
  if(jobs_.empty()) {
    cerr<<"Task::delSubFiles() Error: no jobs defined"<<endl;
    return EXIT_FAILURE;
  }

  for(vector<Job*>::const_iterator i = jobs_.begin(); i!=jobs_.end(); i++) {
    if(Log::level()>2) cout <<"Task::delSubFiles() Deleting submission files for job with id " << (*i)->Id() <<endl;
    if((*i)->delSubFiles()) return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
}
