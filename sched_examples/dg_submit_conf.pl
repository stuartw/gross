...
#    }
    ...
    $inSandBox  .= ",\"BossGeneralInfo_$jid\",\"$ENV{BOSSDIR}/bin/dbUpdator\", \"BossConfig.clad\"";
...
# AF: fix not to loose files in InputSandbox having a name matching
#     other files already in the InputSandbox 
#                       if ( $inSandBox !~ m/$file/ ) {
			if ( $inSandBox !~ m/$file / ) {
			    $inSandBox.=",\"$file\"";
			} 
...
#    exit;
	...
	if ( $_ =~ m/https:(.+)/) {
	    print LOG "$jid: Scheduler ID = https:$1\n";
           $check_cmd="less ~/.bossEDGjobs | grep \"https:$1\" ";
...
#HUGH DBG    unlink "$tmpfile";
#HUGH DBG    unlink "BossGeneralInfo_${jid}";
#HUGH DBG    unlink "BossArchive_${jid}.tgz";
} else {
